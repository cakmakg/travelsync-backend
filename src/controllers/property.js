...
class PropertyController extends BaseController {
  ...
  getByRating = asyncHandler(async (req, res) => {
    ...
    if (ratingNum < 1 || ratingNum > 5) {
      return res.badRequest('Star rating must be between 1 and 5');
    }
   ...
   */
  updateAmenities = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { amenities } = req.body;

    if (!Array.isArray(amenities)) {
      return res.badRequest('Amenities must be an array');
    }
    ...
    if (!property) {
      return res.notFound('Property not found');
    }

    property.amenities = amenities;
    await property.save();

    return res.success(property, { message: 'Amenities updated successfully' });
  });
   ...
   */
  getFullAddress = asyncHandler(async (req, res) => {
    const { id } = req.params;
